...
def map_model(provider: str, requested_model: str) -> str:
    ...
    is_pro = "pro" in model_lower or "sonnet" in model_lower or ("gpt-4o" in model_lower and "mini" not in model_lower)
...
class LLMRouter:
    def __init__(self):
        self.active_provider = os.getenv("ACTIVE_LLM_PROVIDER", "gemini").lower()
        self.keys = {
    ...
    def _get_client(self, provider: str):
        ...
        if not key:
            raise ValueError(f"API key for LLM provider '{provider}' is not set.")
    ...
    async def _call_with_fallback(
        ...
        raise RuntimeError(f"All LLM providers failed. Attempts summary: {'; '.join(errors)}")
    ...
    async def _execute_call(
        ...
        req_model = requested_model or "gemini-2.5-flash"
        ...
        if provider == "gemini":
            ...
            try:
                ...
                logger.warning(f"Gemini AIO failed, using thread executor sync fallback: {aio_err}")
